...
map.on('load', async () => {
  ...
  map.addLayer({
    ...
    paint: {
      'line-color': '#32D400',  // same color to match
      'line-width': 4,
      'line-opacity': 0.6
    }
  });
  ...
  // Load CSV with d3.autoType to convert numbers + dates automatically
  trips = await d3.csv(tripsUrl, d3.autoType);
